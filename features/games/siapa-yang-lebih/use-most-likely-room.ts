...
export type MostLikelyClientAction =
  | "most-likely-answer"
  | "most-likely-continue";

type RoomPayload = MostLikelyRoomView & {
  token?: string;
  error?: string;
};
...
export function useMostLikelyRoom() {
  const [room, setRoom] =
    useState<MostLikelyRoomView | null>(null);
  ...
  const roomRef =
    useRef<MostLikelyRoomView | null>(null);

  const commitRoom = useCallback(
    (nextRoom: MostLikelyRoomView) => {
  ...
  const fetchRoom = useCallback(
    async (code: string, playerToken = "") => {
      ...
      const payload =
        (await response.json()) as RoomPayload;
      ...
      if (payload.gameSlug !== MOST_LIKELY_SLUG) {
        throw new Error(
