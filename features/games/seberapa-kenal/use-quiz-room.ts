...
export function useQuizRoom() {
  const [room, setRoom] =
    useState<QuizRoomView | null>(null);
  ...
  const roomRef = useRef<QuizRoomView | null>(null);

  const commitRoom = useCallback(
    (nextRoom: QuizRoomView) => {
  ...
  const fetchRoom = useCallback(
    async (code: string, playerToken = "") => {
      ...
      if (payload.gameSlug !== COUPLE_QUIZ_SLUG) {
        throw new Error(
